...
class TestObjectSaver < Minitest::Test
  def test_initialize
    assert_equal(ObjectSaver.new(File.expand_path(File.dirname(__FILE__))), ObjectSaver.new(File.expand_path(File.dirname(__FILE__))))
  ...
  def test_that_can_dismember_an_entire_object
    object_saver = ObjectSaver.new File.expand_path(File.dirname(__FILE__))
